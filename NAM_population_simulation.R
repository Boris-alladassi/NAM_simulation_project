nam_simulation <- function(architecture = "A", nfamilies = 10, nfounders = 100, nChrom = 10, totalNqtn = 200,
                           traitMean = 300, traitVar = NULL, traitVA=25, traitVD=0, traitVE=0, traitH2 = 1, 
                           traitName = "Trait1", famSize = 250){
  # traitVA=25; traitVD=0; traitVE=0
  ...
  if(architecture == "A"){
    ###### Additive ---------------- ####
    ###################### Founders
    SP$addTraitA(nQtl = floor(totalNqtn/nChrom), mean = traitMean, var = traitVA, name = traitName)
    ...
    founder_gv <- create_phenotypes(
      ...
      rep = 1,
      add_QTN_num = totalNqtn,
      big_add_QTN_effect = sqrt(traitVA)/2,
      add_effect = -0.9,
    ...
    ###################### Additive + Epistasis ------------------------ ####
    SP$addTraitAE(nQtl = totalNqtn/nChrom, mean = traitMean, var = traitVA, name = traitName, relAA = traitVE/traitVA) 
